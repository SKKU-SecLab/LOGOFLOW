...
warnings.filterwarnings(
    "ignore",
    message=".*align_corners=False.*"
)
warnings.filterwarnings(
    "ignore",
    message=".*non-full backward hook.*"
)
...
class RectifiedFlow(Module):
    ...
    def forward(
        ...
        **model_kwargs
        ...
        # if self.siamese_model is not None:
        #     with torch.no_grad():
        #         target_img = ((data + 1) / 2).clamp(0, 1)
        #         generated_img = ((pred_data +1) / 2).clamp(0, 1)
        #         generated_img.requires_grad_() == False
        ...
        #             attention_map = F.interpolate(attention_map, size=pred_flow.shape[2:], mode="bicubic", align_corners=True)
            
        #         alpha = self.siamese_scale  
            
        #         if sim_score.mean() > 0.7:
        ...
        # === Resizing to match input size ===
        cam_map = cam[0, 0].cpu().numpy()
        H, W = data.shape[2], data.shape[3]
        #print("H, W", H, W)
        cam_resized = cv2.resize(cam_map, (W, H), interpolation=cv2.INTER_CUBIC)
        scale = cam_resized.max() - cam_resized.min()
    
        if scale == 0:
            cam_resized = np.zeros_like(cam_resized) + 1e-6
        else:
            cam_resized = (cam_resized - cam_resized.min()) / (scale + 1e-8)

        attribution_vis = (cam_resized * 255).astype(np.uint8)
        heatmaps = cv2.applyColorMap(attribution_vis, cv2.COLORMAP_JET)

        cam_resized = np.expand_dims(cam_resized, axis=-1)  # (H, W, 1)

        orig_img = ((data[0].detach().cpu().permute(1, 2, 0) + 1) / 2).clamp(0, 1).numpy()
        #print("orig img size:", orig_img.shape)
        ...
        fig, _ = viz.visualize_image_attr_multiple(
            ...
            titles=["Original Image", "GradCAM Heatmap", "Positive Heatmap"],
            show_colorbar=True
        )

       
        save_dir = "gradcam_rectifiedflow"
        os.makedirs(save_dir, exist_ok=True)
        save_path = os.path.join(save_dir, "rectified_cam_check.png")

        fig.savefig(save_path)
        plt.close(fig)

        original_image = (orig_img * 255).astype(np.uint8)
        original_image = cv2.resize(original_image, (W, H))
        alpha = 0.6
        overlay = cv2.addWeighted(original_image, 1 - alpha, heatmaps, alpha, 0)
        heatmap_with_text = overlay.copy()
        font = cv2.FONT_HERSHEY_SIMPLEX
        step_size = 20 
        for y in range(0, cam_resized.shape[0], step_size):
            for x in range(0, cam_resized.shape[1], step_size):
                value = float(cam_resized[y, x])
                if value > 0.5:  
                    text = f"{value:.2f}"
                    cv2.putText(heatmap_with_text, text, (x, y), font, 0.3, (255, 255, 255), 1, cv2.LINE_AA)
        save_path_ = os.path.join(save_dir, "rectified_cam_heatmap.png")
        fig, ax = plt.subplots(1, 2, figsize=(12, 6))
        ax[0].imshow(cv2.cvtColor(overlay, cv2.COLOR_BGR2RGB))
        ax[0].set_title("Heatmap Overlay")
        ax[0].axis("off")

        ax[1].imshow(cv2.cvtColor(heatmap_with_text, cv2.COLOR_BGR2RGB))
        ax[1].set_title("Heatmap with Attribution Values")
        ax[1].axis("off")
        ...
        siamese_map = np.load("siam_gradcam_map/attention_map_atnt.npy")  # (H, W)
        siamese_map = (siamese_map - siamese_map.min()) / (siamese_map.max() - siamese_map.min() + 1e-8)
        siamese_map = torch.from_numpy(siamese_map).float().to(device)  # [H, W]

        rectified_map = torch.from_numpy(cam_resized).float().squeeze().to(device)  # [H, W]

        if siamese_map.shape != rectified_map.shape:
            H, W = rectified_map.shape  
            siamese_map = F.interpolate(
                siamese_map.unsqueeze(0).unsqueeze(0),  # [1, 1, Hs, Ws]
                size=(H, W),  
                mode="bilinear",
                align_corners=False
            ).squeeze(0).squeeze(0)  # [H, W]

        cam_loss = F.mse_loss(rectified_map, siamese_map)
        ...
        if self.siamese_model is not None:
            ...
            # hinge loss style
            margin = torch.tensor(0.9, device=device)
            offset = torch.tensor(0.05, device=device)
            zero = torch.tensor(0.0, device=device)
                        
            siamese_loss = torch.max(zero, siamese_distance - margin + offset).mean()
            # threshold = 0.7
            # siamese_loss = torch.relu(siamese_distance - threshold).mean()        
        ...
        gradcam_hyper = 0.5
        # hyper = 0.000001
        total_loss = main_loss + consistency_loss * self.consistency_loss_weight + siamese_hyper * siamese_loss + gradcam_hyper * cam_loss
        log_line = (
            f"main_loss: {main_loss.item():.6f}, "
            f"siamese_loss: {siamese_loss:.6f}, "
            f"cam_loss: {cam_loss:.6f}, "
