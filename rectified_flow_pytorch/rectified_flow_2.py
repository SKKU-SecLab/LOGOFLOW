...
class RectifiedFlow(Module):
    def __init__(
        ...
        #    self.siamese_model.siamese_model = self.siamese_model.siamese_model.to(self.device)
        #    self.siamese_model.ocr_model = self.siamese_model.ocr_model.to(self.device)
    ...
    def forward(
        ...
        **model_kwargs
        ...
        #        margin = 0.8
        #        offset = 0.1
        #        siamese_loss = torch.max(
        #            torch.tensor(0.0).to(self.device),
        #            siamese_distance - torch.tensor(margin).to(self.device) + torch.tensor(offset).to(self.device)
        #        ).mean()
