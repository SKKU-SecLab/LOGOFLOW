...
# from .siamese_model2 import siamese_loss
...
class GaussianDiffusion:
    ...
    def __init__(
        ...
        *,
        ...
        self.betas = betas
        # print("what is betas ======", betas)
        assert len(betas.shape) == 1, "betas must be 1-D"
        assert (betas > 0).all() and (betas <= 1).all()

        self.num_timesteps = int(betas.shape[0])
        print("what is num_timesteps beta shape ", self.num_timesteps)
        self.interpolate_batch = 6
        print("interpolate batch for testing ", self.interpolate_batch)
        self.mix_rate = 0.5
    ...
    def p_sample_loop_progressive(
        ...
        for i in indices:
            # print("what is i ", i)
            # print("what is shape 0", shape[0])
            # print("what is i * shape[0]", [i]*shape[0])
            ...
            with th.no_grad():
                ...
                #         xt_sample = ((img+ 1) * 127.5).clamp(0, 255).to(th.uint8)
                #         xt_sample = xt_sample.permute(0, 2, 3, 1)
                #         xt_img =  Image.fromarray(xt_sample.cpu().numpy()[index])
                #         xt_img.save("./middle/middle_{}_{}.png".format(count, index))
    ...
    def interpolate(
        ...
        # reference: https://github.com/sak-h/pytorch-Denoising-Diffusion-Probabilistic-Models/blob/master/models/ddpm.py
        if device is None:
            device = next(model.parameters()).device
        ...
        with torch.no_grad():
            ...
            xt_1 = self.q_sample(
                x0_1,
                torch.full(
                    tuple([B]), self.num_timesteps - 1, dtype=torch.long, device=device
                ),
            )
            xt_2 = self.q_sample(
                x0_2,
            ...
            xt_mix = (
                torch.tensor((1.0 - self.mix_rate), dtype=torch.float32, device=device)
                * xt_1
                + torch.tensor(self.mix_rate, dtype=torch.float32, device=device) * xt_2
            )
            for i in reversed(range(self.num_timesteps - 1)):
                out = self.p_sample(
                    model,
                    xt_mix.detach(),
                    t=torch.full(tuple([B]), i, dtype=torch.long, device=device),
                    clip_denoised=clip_denoised,
                    denoised_fn=denoised_fn,
                    cond_fn=cond_fn,
                    model_kwargs=model_kwargs,
    ...
    def training_losses(
        ...
        # def training_losses(self, model, siamese_model, x_start, t, model_kwargs=None, noise=None):
        """
        ...
        print("saving x_t for time step ", t)
        xt_sample = ((x_t + 1) * 127.5).clamp(0, 255).to(th.uint8)
        print("x sample again shape ", xt_sample.shape)
        xt_sample = xt_sample.permute(0, 2, 3, 1)
        print("x sample again after permutation ", xt_sample.shape)
        print("type ", type(xt_sample.cpu().numpy()))
        xt_img = Image.fromarray(xt_sample.cpu().numpy()[0])
        # print("noise does it have gradients", noise)x
        xt_img.save("./xt/xt_img_{}.png".format(t))

        batch_size = x_start.shape[0]
        # print("batch size========= ", batch_size)
        terms = {}
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            terms["loss"] = self._vb_tedrms_bpd(
                model=model,
                x_start=x_start,
                x_t=x_t,
                t=t,
            ...
            # maybe because here there is some links
            # target2 = (target + 1) / 2
            # model_output2 = (model_output + 1) / 2
            # print("target gradients ", target)
            # print("model output gradients", model_output)

            target2 = ((target + 1) / 2).clamp(0, 1)
            model_output2 = ((model_output + 1) / 2).clamp(0, 1)

            # target2_masked_img = target2 * mask
            # model_output2_masked_img = model_output2 * mask

            # print("target2 shape ", target2.shape)
            # print("model output2 shape ", mask.shape)

            # target2 = ((target+1)/ 2 * 255).clamp(0, 255).to(th.uint8)
            # model_output2 = ((model_output+1) * 255).clamp(0, 255).to(th.uint8)
            # target2 = target2.permute(0, 2, 3, 1)
            # model_output2 = model_output2.permute(0, 2, 3, 1)
            # target2 = target2.contiguous()
            # model_output2 = model_output2.contiguous()
            # print("target2 shape ", target2.shape)
            # print("modle output2 shape ", model_output2.shape)

            siamese_difference = siamese_model.siamese_loss(target2, model_output2)

            # margin and confidence
            siamese_distance_total = th.max(
                th.tensor(0.0).to(target.device),
                siamese_difference
                - th.tensor(0.8).to(target2.device)
                + th.tensor(0.1).to(target2.device),
            )

            # siamese_distance_total = torch.relu(siamese_difference - th.tensor(0.8).to(target2.device) + th.tensor(0.1).to(target2.device))
            terms["siamese"] = siamese_distance_total * scaling_factor
            # print("scaling factor ", scaling_factor)
            # print("why it has values? ", scaling_factor, siamese_distance_total, siamese_distance_total * scaling_factor)
            print("terms[siamese] correct distance ", siamese_distance_total)
            # print("terms[siamese] distance ", terms["siamese"])
            # print("terms[siamese] wrong distance ", siamese_distance_total_wrong)
            print("terms[mse] distance ", terms["mse"])
            print("terms[vb] distance ", terms["vb"])
            # print("making dot for siamese network")
            # make_dot(terms["siamese"], params=dict(list(model.named_parameters()))).render("torchviz", format="png")

            if "vb" in terms:
                terms["loss"] = terms["mse"] + terms["vb"] + terms["siamese"]
                # terms["loss"] = terms["mse"] + terms["vb"]
                # terms["loss"] = terms["siamese"]
            else:

                terms["loss"] = terms["mse"] + terms["siamese"]
