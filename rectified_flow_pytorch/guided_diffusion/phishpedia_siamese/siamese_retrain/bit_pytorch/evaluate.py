...
def evaluate_special(model, dataset):
    ...
    for j in range(len(dataset)):
        ...
        img_coords = np.asarray(dataset.preprocess_coordinates)[
            np.asarray(dataset.paths) == filepath
        ]  # box coordinates
        img_classes = np.asarray(dataset.img_classes)[
            np.asarray(dataset.paths) == filepath
        ]  # box types
        # print(img_coords)
        ...
        if pred_cls != y:
            print(
                "File {} has predicted class {:d} with predicted score {:.2f}".format(
                    filepath, pred_cls, pred_score[:, pred_cls].item()
                )
            )
            ...
            cv2.imwrite(
                os.path.join(
                    ...
                    filepath
                    + "_{}_{}_".format(
                        "credential" if pred_cls == 0 else "noncredential",
                        str(pred_score[:, pred_cls].item()),
                    )
                    + ".png",
                ),
                img,
            ...
            heatmap_vis(
                x, os.path.join(bad_example_dir, filepath + "_heatmap_xywh_c5_.png")
            )
...
def cls_avg_heatmaps(cls, dataset):
    ...
    heatmap_vis(
        avg_channel_heatmaps, savedir="./output/cls{}_avg_heatmap.png".format(str(cls))
    )
...
if __name__ == "__main__":
    ...
    val_loader = torch.utils.data.DataLoader(
        val_set, batch_size=len(val_set), drop_last=False, shuffle=False
    )

    model = models.KNOWN_MODELS["SE-R50x32"](head_size=len(val_set.classes))
    checkpoint = torch.load(
        "output/se_targetlist_lr0.01/bit.pth.tar", map_location="cpu"
    )["model"]
