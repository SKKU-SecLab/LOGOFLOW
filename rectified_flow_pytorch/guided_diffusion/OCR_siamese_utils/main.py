...
def main(args):
    ...
    if not args.evaluate:
        ...
        make_symlink_if_not_exists(
            osp.join(args.real_logs_dir, args.logs_dir),
    ...
    test_dataset, test_loader = get_data(
        args.test_data_dir,
        args.voc_type,
        args.max_len,
        args.num_test,
    ...
    if args.resume:
        ...
        if "epoch" in checkpoint.keys():
            ...
            start_epoch = (
                int(start_iters // len(train_loader)) if not args.evaluate else 0
            )
    ...
    # Start training
    evaluator.evaluate(
        test_loader, step=0, tfLogger=eval_tfLogger, dataset=test_dataset
    )
