...
def siamese_inference_OCR(
    ...
    img_feat = pred_siamese_OCR(
        cropped,
        model,
        ocr_model,
        imshow=False,
        title="Original rcnn box",
        grayscale=grayscale,
    )
    ...
    for j in range(3):
        ...
        ## Else if not exceed, try resolution alignment, see if can improve
            ...
            img_feat = pred_siamese_OCR(
                cropped, model, ocr_model, imshow=False, title=None, grayscale=grayscale
            )
            logo_feat = pred_siamese_OCR(
                candidate_logo,
                model,
                ocr_model,
                imshow=False,
