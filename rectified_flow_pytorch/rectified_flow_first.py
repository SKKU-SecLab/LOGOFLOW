    ...
    def forward(
        ...
        **model_kwargs
        ...
        if self.siamese_model is not None:
            ...
            generated_img = ((pred_data + 1) / 2).clamp(0, 1)
            # generated_img = generated_img.detach().requires_grad_(False)  
            # if hasattr(self.siamese_model, 'rnn_layer'):
            #     # printI("hasarrt")
            #     self.siamese_model.rnn_layer.train()

            # generated_img.requires_grad_(True)
            # with torch.no_grad():
            # print("siamese dist")
            # self.ocr_model.train()  # OCR 모델이 RNN을 포함한다면
        ...
        if self.siamese_model is not None:
            with torch.no_grad():
                ...
                pixel_diff = F.l1_loss(target_img, generated_img, reduction='none')
                pixel_diff = pixel_diff.mean(dim=1, keepdim=True)
                
                    
                self.siamese_model.visualize_attention_map(
                    attention_map,
                    save_path=f"./attention_map_step.png", 
                    idx=0,
                    title=f"Attention Map (sim: {sim_score[0].item():.2f})"
                )
                
                attention_weight = attention_map
                #print("attention_weight shape:", attention_weight.shape)
                
                #xai_guided_loss = (pixel_diff * attention_weight).mean()
                if sim_score.mean() > 0.7:
                    xai_guided_loss = (pixel_diff * attention_weight).mean()
                else:
                    xai_guided_loss = (pixel_diff * (1 - attention_weight)).mean()"""
