...
class Trainer(Module):
    ...
    def forward(self, start_step: int = 0):
        ...
        for ind in range(start_step, self.num_train_steps):
            ...
            #         for name, param in self.model.module.siamese_model.named_parameters()                    
            #     }
            ...
            # if getattr(base, 'use_consistency', False):  #
            #     base.ema_model.update()

            if self.is_main and self.use_ema:
                # self.ema_model.ema_model.data_shape = self.model.data_shape
                model_unwrapped = getattr(self.model, "module", self.model)
                self.ema_model.ema_model.data_shape = model_unwrapped.data_shape
                # self.ema_model.ema_model.data_shape = base.data_shape
